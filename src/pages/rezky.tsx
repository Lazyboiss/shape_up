...
export const PlatformerGame: React.FC<PlatformerGameProps> = ({ lines = [] }) => {
  ...
  useEffect(() => {
    ...
    Matter.Events.on(engine, 'collisionStart', (event) => {
      event.pairs.forEach((pair) => {
        const isPlayer = pair.bodyA === playerRef.current || pair.bodyB === playerRef.current;
        if (isPlayer) {
          // Get the collision normal relative to the player
          const normal = pair.bodyA === playerRef.current ? pair.collision.normal : {
    ...
    Matter.Events.on(engine, 'collisionActive', (event) => {
      let foundGroundCollision = false;
      event.pairs.forEach((pair) => {
        const isPlayer = pair.bodyA === playerRef.current || pair.bodyB === playerRef.current;
        if (isPlayer) {
          const normal = pair.bodyA === playerRef.current ? pair.collision.normal : {
    ...
    const gameLoop = () => {
      ...
      const noKeysPressed = !keysRef.current.left && !keysRef.current.right && !keysRef.current.space;
    ...
    <div>
      ...
      {firstClickPoint && (
        <div style={{ 
          ...
          First point set at ({Math.round(firstClickPoint.x)}, {Math.round(firstClickPoint.y)}). 
          Click again to create platform.
        </div>
      )}
    </div>
  );
