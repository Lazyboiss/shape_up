...
type ToolMode = "select" | "platform" | "player1Flag" | "player2Flag" | "delete";
...
export const PlatformerGame: React.FC<PlatformerGameProps> = ({
  ...
  const player1LockedPositionRef = useRef<{ x: number; y: number } | null>(null);
  const player2LockedPositionRef = useRef<{ x: number; y: number } | null>(null);
  ...
  useEffect(() => {
    ...
    Matter.Events.on(engine, "collisionStart", (event) => {
      event.pairs.forEach((pair) => {
        ...
        const isPlayer1 = pair.bodyA === player1Ref.current || pair.bodyB === player1Ref.current;
        if (isPlayer1) {
          const otherBody = pair.bodyA === player1Ref.current ? pair.bodyB : pair.bodyA;
          
          // Check if Player 1 touched any Player 1 flag
          flagsRef.current.forEach((flagData, index) => {
            if (flagData.playerType === 1 && otherBody === flagData.flag && !flagData.raised) {
          ...
          if (!otherBody.isSensor) {
            const normal = pair.bodyA === player1Ref.current
        ...
        const isPlayer2 = pair.bodyA === player2Ref.current || pair.bodyB === player2Ref.current;
        if (isPlayer2) {
          const otherBody = pair.bodyA === player2Ref.current ? pair.bodyB : pair.bodyA;
          
          // Check if Player 2 touched any Player 2 flag
          flagsRef.current.forEach((flagData, index) => {
            if (flagData.playerType === 2 && otherBody === flagData.flag && !flagData.raised) {
              flagData.raised = true;
              setFlagStates(prev => ({ ...prev, [`flag_${index}`]: true }));
            }
          });
          ...
          if (!otherBody.isSensor) {
            const normal = pair.bodyA === player2Ref.current
              ? pair.collision.normal
              : { x: -pair.collision.normal.x, y: -pair.collision.normal.y };
    ...
    Matter.Events.on(engine, "collisionEnd", (event) => {
      event.pairs.forEach((pair) => {
        if (pair.bodyA === player1Ref.current || pair.bodyB === player1Ref.current) {
          player1GroundedRef.current = false;
          player1GroundNormalRef.current = null;
          player1LockedPositionRef.current = null;
        }
        if (pair.bodyA === player2Ref.current || pair.bodyB === player2Ref.current) {
          player2GroundedRef.current = false;
          player2GroundNormalRef.current = null;
          player2LockedPositionRef.current = null;
        }
    ...
    const gameLoop = () => {
      ...
      const player1NoKeys = !keysRef.current.a && !keysRef.current.d && !keysRef.current.w;
      
      applySlopeStick(player1Ref.current, player1GroundNormalRef.current, player1GroundedRef.current, player1Moving);
      ...
      if (keysRef.current.a) {
        ...
        Matter.Body.setPosition(player1Ref.current, player1LockedPositionRef.current);
      ...
      if (keysRef.current.w && player1GroundedRef.current) {
        Matter.Body.applyForce(player1Ref.current, player1Ref.current.position, {
          x: 0,
          y: jumpForce,
        });
      ...
      const player2NoKeys = !keysRef.current.left && !keysRef.current.right && !keysRef.current.up;
      
      applySlopeStick(player2Ref.current, player2GroundNormalRef.current, player2GroundedRef.current, player2Moving);
      ...
      if (keysRef.current.left) {
        ...
        Matter.Body.setPosition(player2Ref.current, player2LockedPositionRef.current);
      ...
      if (keysRef.current.up && player2GroundedRef.current) {
        Matter.Body.applyForce(player2Ref.current, player2Ref.current.position, {
          x: 0,
          y: jumpForce,
        });
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (toolMode === "platform") {
      ...
      setFlagStates(prev => ({ ...prev, [`flag_${flagsRef.current.length - 1}`]: false }));
      ...
      for (const body of bodies) {
        ...
        if (flagIndex !== -1) {
          ...
          Matter.World.remove(engineRef.current.world, [flagData.pole, flagData.flag]);
  ...
  const getCursor = () => {
    ...
    if (toolMode === "platform" || toolMode === "player1Flag" || toolMode === "player2Flag") return "pointer";
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <div style={{
        ...
        {menuOpen && (
          <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
            <button
              onClick={() => { setToolMode("select"); setFirstClickPoint(null); }}
              style={{
                padding: "8px",
            ...
            <button
              onClick={() => { setToolMode("platform"); setFirstClickPoint(null); }}
              style={{
                padding: "8px",
                backgroundColor: toolMode === "platform" ? "#9B59B6" : "#f0f0f0",
            ...
            <button
              onClick={() => { setToolMode("player1Flag"); setFirstClickPoint(null); }}
              style={{
                padding: "8px",
                backgroundColor: toolMode === "player1Flag" ? "#00FF00" : "#f0f0f0",
            ...
            <button
              onClick={() => { setToolMode("player2Flag"); setFirstClickPoint(null); }}
              style={{
                padding: "8px",
                backgroundColor: toolMode === "player2Flag" ? "#FFA500" : "#f0f0f0",
            ...
            <button
              onClick={() => { setToolMode("delete"); setFirstClickPoint(null); }}
              style={{
                padding: "8px",
      ...
      {gameWon && (
        ...
        >
          <h1 style={{ fontSize: "72px", margin: "20px", textShadow: "3px 3px 6px rgba(0,0,0,0.8)" }}>
            You Win!
          </h1>
      ...
      <div style={{ textAlign: "center", marginTop: "10px", fontSize: "14px" }}>
        <div style={{ marginBottom: "5px" }}>
          <span style={{ color: "#00FF00", fontWeight: "bold" }}>Player 1 (Green)</span>: A = Left, D = Right, W = Jump
        </div>
        <div>
          <span style={{ color: "#FFA500", fontWeight: "bold" }}>Player 2 (Orange)</span>: ← = Left, → = Right, ↑ = Jump
        </div>
      </div>
    </div>
  );
