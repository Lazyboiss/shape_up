...
export default function PlatformerGame() {
  const [currentPage, setCurrentPage] = useState('levelSelect');
...
function LevelSelectPage({ onLevelSelect, showInstructions, setShowInstructions }) {
  return (
...
function LevelButton({ label, color, hoverColor, onClick, delay }) {
  return (
    <button
      onClick={onClick}
...
// Game Page Component
function GamePage({ level, onReturnToLevelSelect }) {
  ...
  const initGame = () => {
    if (!window.kaboom || gameRef.current) return;
    ...
    // Load current level
    const levelMap = levels[level] || levels.easy;
    const tileWidth = 1200 / levelMap[0].length;
    const tileHeight = 600 / levelMap.length;

    // Track player states
    let player1ReachedFlag = false;
    let player2ReachedFlag = false;
    let gameEnded = false;
    let timer = 60;
